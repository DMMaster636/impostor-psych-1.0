...
class PlayState extends MusicBeatState
{
	...
	public var extraCharMap:Map<String, Character> = new Map<String, Character>();
	...
	public var EXTRACHAR_X:Float = 100;
	public var EXTRACHAR_Y:Float = 100;
	...
	var noteRows:Array<Array<Array<Note>>> = [[/*0 = bf*/], [/*1 = dad*/], [/*2 = gf*/], [/*3 = extraChar*/]];
	...
	public var extraCharCameraOffset:Array<Float> = null;

	public var stagePlayerCamOffset:Array<Float> = null;
	public var stageOpponentCamOffset:Array<Float> = null;
	public var stageGirlfriendCamOffset:Array<Float> = null;
	public var stageExtraCharCamOffset:Array<Float> = null;
	...
	#if SCRIPTS_ALLOWED private var luaDebugGroup:FlxTypedGroup<psychlua.DebugLuaText>; #end
	...
	override public function create()
	{
		...
		clearSongSyncThread();
		syncThread = new FixedThreadPool(1);
		...
		extraCharCameraOffset = stageData.camera_extraChar;
		if(extraCharCameraOffset == null) extraCharCameraOffset = [0, 0];

		stagePlayerCamOffset = stageData.cam_pos_player;
		stageOpponentCamOffset = stageData.cam_pos_opponent;
		stageGirlfriendCamOffset = stageData.cam_pos_gf;
		stageExtraCharCamOffset = stageData.cam_pos_extraChar;
		...
		switch (curStage)
		{
			case 'stage': new StageWeek1(); 			// Default Stage
			case 'polus': new PolusRed(); 				// Week 1 - Red
			case 'toogus': new MiraGreen(); 			// Week 2 - Green
			case 'airshipRoom': new AirshipCockpit(); 	// Week 3 - Yellow/White
			case 'airship': new AirshipOutside(); 		// Week 3 - Black
			//case 'stage': new StageWeek1(); 			// Week 4 - ???
			//case 'stage': new StageWeek1(); 			// Week 5 - ???
			//case 'stage': new StageWeek1(); 			// Week 6 - ???
			//case 'stage': new StageWeek1(); 			// Week 7 - ???
			//case 'stage': new StageWeek1(); 			// Week 8 - ???
			case 'school': new SkeldMongusPixel(); 		// Week 9 - Tomongus Pixel
			case 'kills': new OwStage(); 				// Freeplay - Ow
		}
		...
		{
			extraChar = new Character(0, 0, SONG.player4);
			startCharacterPos(extraChar);
			extraCharGroup.add(extraChar);
			extraChar.cameraOffset = extraCharCameraOffset;
		}
		...
		{
			var list:Map<String, FlxSprite> = StageData.addObjectsToState(stageData.objects, !stageData.hide_girlfriend ? gfGroup : null, dadGroup, boyfriendGroup, extraCharGroup, this);
			for (key => spr in list)
		...
		if(extraChar != null) startCharacterScripts(extraChar.curCharacter);
		#end
		...
		timeTxt = new FlxText(60, 22, 400, "", 24);
		timeTxt.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		timeBar = new Bar(40, 26, 'timeBar', null, function() return songPercent, 0, 1);
		timeBar.scrollFactor.set();
		timeBar.setColors(0xFF44d844, 0xFF2e412e);
		timeBar.barOffset.set(5, 5);
		timeBar.barWidth = Std.int(timeBar.bg.width - timeBar.barOffset.x * 2);
		timeBar.barHeight = Std.int(timeBar.bg.height - timeBar.barOffset.y * 2);
		...
		var infoFile:String = Paths.txt('$songName/info_${ClientPrefs.data.language}');
		if(!Paths.fileExistsAbsolute(infoFile)) infoFile = Paths.txt('$songName/info');

		if(Paths.fileExistsAbsolute(infoFile))
		{
			trace('song infos exist!');
		...
		if(!ClientPrefs.data.comboGameCam)
	...
	public function addTextToDebug(text:String, ?color:FlxColor = FlxColor.RED, ?error:Bool = false) {
		#if SCRIPTS_ALLOWED
	...
	public function reloadScoreTextColors() {
	...
	public function addCharacterToList(newCharacter:String, type:Int) {
		switch(type) {
			...
			case 3:
				if(extraChar != null && !extraCharMap.exists(newCharacter)) {
					var newExtraChar:Character = new Character(0, 0, newCharacter);
					extraCharMap.set(newCharacter, newExtraChar);
					extraCharGroup.add(newExtraChar);
					startCharacterPos(newExtraChar);
					newExtraChar.alpha = 0.00001;
					newExtraChar.cameraOffset = extraCharCameraOffset;
	...
	public function addBehindExtraChar(obj:FlxBasic)
	{
		insert(members.indexOf(extraCharGroup), obj);
	}
	...
	function startSong():Void
	{
		...
		#if DISCORD_ALLOWED
		switch (dad.curCharacter.toLowerCase().trim())
		{
			case "impostor": curPortrait = "red"; // red
			case "impostor2": curPortrait = "red"; // green
			case "crewmate": curPortrait = "crewmate";
			case "impostor3": curPortrait = "greenimp";
			case "impostorr": curPortrait = "greenimp";
			case "tomongus": curPortrait = "tomongus";
			case "idk": curPortrait = "idk";
			case "chewmate": curPortrait = "tomongus";
			case "black": curPortrait = "black";
			case "charles": curPortrait = "charles";
			case "henry": curPortrait = "henry";
			case "monotone": curPortrait = "shifter";
			case "rhm": curPortrait = "armed";
			case "fella": curPortrait = "loggo";
			case "boo": curPortrait = "loggo";
			case "parasite": curPortrait = "parasite";
			case "nuzzus": curPortrait = "nuzzus";
			case "drippypop": curPortrait = "drippypop";
			case "tuesday": curPortrait = "tuesday";
			case "pink": curPortrait = "pink";
			case "grey": curPortrait = "grey";
			case "yellow": curPortrait = "yellow";
			case "white": curPortrait = "white";
			case "jorsawsee": curPortrait = "jorsawsee";
			case "warchief": curPortrait = "warchief";
			case "powers": curPortrait = "powers";
		}

		switch (curSong.toLowerCase().trim())
		{
			case "double-kill": curPortrait = "doublekill";
			case "reinforcements": curPortrait = "ellie";
			case "turbulence": curPortrait = "turbulence";
			case "voting-time": curPortrait = "votingtime";
			case "torture": curPortrait = "torture";
			case "who": curPortrait = "who";
		}

		// Updating Discord Rich Presence (with Time Left)
		if(autoUpdateRPC) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength, curPortrait);
		#end
	...
	private function generateSong():Void
	{
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				// let's make sure event notes don't get created as normal notes...
				for (eventNote in unspawnNotes)
				{
					if (eventNote.noteData >= 0) continue;

					eventNote.destroy();
					unspawnNotes.remove(eventNote);
					noteRows[eventNote.mainRow][eventNote.subRow].remove(eventNote);
					badEventsCaught++;
				}
				...
				if (i != 0)
				{
					...
					for (evilNote in unspawnNotes)
					{
						...
						if (matches && Math.abs(spawnTime - evilNote.strumTime) < FlxMath.EPSILON)
						{
							...
							noteRows[evilNote.mainRow][evilNote.subRow].remove(evilNote);
							ghostNotesCaught++;
						}
					}
				}
				...
				swagNote.mainRow = gottaHitNote ? 0 : 1;
		...
		trace('Bad Events Cleared: $badEventsCaught');
	...
	function eventPushed(event:EventNote) {
		...
		switch(event.event) {
			case 'Reactor Beep':
				flashSprite = new FlxSprite().makeGraphic(FlxG.width * 2, FlxG.height * 2, 0xFFb30000);
	...
	override function openSubState(SubState:FlxSubState)
	{
		...
		if (paused)
		{
			...
			final characters:Array<Character> = [boyfriend, gf, dad, extraChar];
	...
	override function closeSubState()
	{
		...
		if (paused)
		{
			...
			FlxG.sound.list.forEachAlive(function(sound:FlxSound) {
				@:privateAccess
				if (sound != null && sound._paused) sound.resume();
			});
	...
	override public function onFocusLost():Void
	{
		...
		if (!paused && !isDead && autoUpdateRPC && ClientPrefs.data.autoPause)
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, 0, curPortrait);
		#end
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		if (!showTime) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, 0, curPortrait);
		else DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset, curPortrait);
		#end
	}
	...
	// https://github.com/ShadowMario/FNF-PsychEngine/pull/16273
	// modified by DM-kun
	private static var syncThread:FixedThreadPool = null;
	...
	{
		if(!ClientPrefs.data.songResync) return;
	...
	public function clearSongSyncThread()
	{
		if(!ClientPrefs.data.songResync) return;

		shutdownThread = true;
		if(syncThread != null) syncThread.shutdown();
		syncThread = null;
	...
	{
		if(flashSprite != null && flashSprite.alpha != 0)
			flashSprite.alpha = FlxMath.lerp(flashSprite.alpha, 0, FlxMath.bound(elapsed * 9 * playbackRate, 0, 1));
	...
	function openPauseMenu()
	{
		...
		#if DISCORD_ALLOWED
		if(autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, 0, curPortrait);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				if(autoUpdateRPC) DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), false, 0, curPortrait);
				#end
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, strumTime:Float) {
		addTextToDebug('"triggerEventNote" is deprecated! Use "triggerEvent" instead!', FlxColor.ORANGE);
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName)
		{
				...
				switch(charType) {
					...
					case 3:
						if(extraChar != null)
						{
							...
							{
								if(!extraCharMap.exists(value2))
									addCharacterToList(value2, charType);

								var lastAlpha:Float = extraChar.alpha;
								extraChar.alpha = 0.00001;
								extraChar = extraCharMap.get(value2);
								extraChar.alpha = lastAlpha;
							}
							setOnScripts('extraCharName', extraChar.curCharacter);
						}
				}
				reloadHealthBarColors();
				reloadScoreTextColors();
			...
			case 'Reactor Beep':
				if(flValue1 == null) flValue1 = 0.4;
				if(!ClientPrefs.data.flashing) flValue1 /= 2;

				flashSprite.alpha = flValue1;
				if(value2 != null && value2.length > 0)
					flashSprite.color = FlxColor.fromString(value2);

				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.03;

			case 'HUD Fade':
				var fadeType:Int = Std.parseInt(value1);
				if(Math.isNaN(fadeType)) fadeType = 0;
				if(flValue2 == null) flValue2 = 0.7;

				switch(fadeType)
				{
					case 0:
						if(flValue2 <= 0) camHUD.alpha = 1;
						else FlxTween.tween(camHUD, {alpha: 1}, flValue2, {ease: FlxEase.quadInOut});
					case 1:
						if(flValue2 <= 0) camHUD.alpha = 0;
						else FlxTween.tween(camHUD, {alpha: 0}, flValue2, {ease: FlxEase.quadInOut});
				}
		}
	...
	{
		if(stageGirlfriendCamOffset != null) camFollow.setPosition(stageGirlfriendCamOffset[0], stageGirlfriendCamOffset[1]);
		else
		{
	...
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
			//stageExtraCharCamOffset
			if(stageOpponentCamOffset != null) camFollow.setPosition(stageOpponentCamOffset[0], stageOpponentCamOffset[1]);
			else
			{
				if(dad == null) return;
		...
		{
			if(stagePlayerCamOffset != null) camFollow.setPosition(stagePlayerCamOffset[0], stagePlayerCamOffset[1]);
			else
			{
	...
	public function moveCamOffset()
	{
		var char:Character = null;
		if (SONG.notes[curSection].gfSection) char = gf;
		else if (!SONG.notes[curSection].mustHitSection) char = dad;
		else char = boyfriend;

		if(char == null || char.animation.curAnim == null || !camMovement) return;

		FlxG.camera.targetOffset.set(0, 0);

		final anim:String = char.getAnimationName();
		if(anim.endsWith('miss')) return;

		if(anim.startsWith('singUP')) FlxG.camera.targetOffset.y = -camOffset;
		else if(anim.startsWith('singDOWN')) FlxG.camera.targetOffset.y = camOffset;
		else if(anim.startsWith('singLEFT')) FlxG.camera.targetOffset.x = -camOffset;
		else if(anim.startsWith('singRIGHT')) FlxG.camera.targetOffset.x = camOffset;
	}
	...
	public function endSong()
	{
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				switch(songName)
				{
					case 'meltdown': ClientPrefs.data.unlockedWeek[1] = true;
					case 'double-kill': ClientPrefs.data.unlockedWeek[3] = true;
					case 'boiling-point': ClientPrefs.data.unlockedWeek[5] = true;
					case 'neurotic': ClientPrefs.data.unlockedWeek[6] = true;
					case 'titular': ClientPrefs.data.unlockedWeek[8] = true;
					case 'ejected': ClientPrefs.data.unlockedWeek[2] = ClientPrefs.data.unlockedWeek[8] = ClientPrefs.data.unlockedWeek[10] = true;
				}
	...
	{
		final placementX:Float = ClientPrefs.data.comboGameCam ? (gf != null ? gf.x : boyfriend.x) : FlxG.width * 0.35;
		final placementY:Float = ClientPrefs.data.comboGameCam ? (gf != null ? gf.y : boyfriend.y) : FlxG.height * 0.35;
